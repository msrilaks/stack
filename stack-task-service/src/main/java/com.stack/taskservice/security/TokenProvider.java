...
public class TokenProvider {
    ...
    public String createToken(Authentication authentication) {
        logger.info("### SRI : " + authentication.getPrincipal() + "   " +
                    authentication.getPrincipal().getClass());
        String email =
                (String) ((OAuth2User) authentication.getPrincipal()).getAttributes().get(
                        "email");
        logger.info("### SRI  email: " + email);
        ...
        return Jwts.builder()
                   .setSubject(email)//Long.toString(userPrincipal.getId()))
